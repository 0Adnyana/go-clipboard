...
| Accounts and ownership | [5](slices/05-accounts-and-ownership.md) | Email/password accounts, cookie sessions, `owner_id` on clips, longer lifetimes, plus login-attempt and per-account creation limiters |
| Account recovery | [6](slices/06-account-recovery.md) | Password reset, verification email, and a verify endpoint; introduces mail; completing a reset evicts every session |
...
| Admin observability / analytics | [9](slices/09-admin-observability.md) | Read-only panel over public and aggregate data plus sweeper health; no 2FA gate, because it neither acts nor exposes anyone |
| Two-factor | [10](slices/10-two-factor.md) | TOTP with hashed single-use recovery codes |
| OAuth | [11](slices/11-oauth.md) | Provider sign-in that auto-links only when both sides assert a verified email |
| Abuse response | [12](slices/12-abuse-response.md) | Unauthenticated reports, accounts-first panel, suspension, append-only audit log; behind 2FA; the publishability gate |
| Challenge layer | [13](slices/13-challenge-layer.md) | A challenge gate as a third defence tier over rate limiting: on suspicious anonymous registration and on anonymous creation |
| Files | [14](slices/14-files.md) | A file is a clip: same pool, lease, ceiling, and `/<slug>`. Text or file, never both. Last, and still provisional |
...
**In.** Text (files last, slice 14). Two browsers, or a browser and a terminal. One instance, one
...
**Nothing before slice 12 is publishable** — which since slice 4 is no longer the same as deployable.
Slice 4 is the MVP and the deployable gate; slice 3 slows an anonymous flood; slice 12 is the first
...
- **The server can read private clip content.** Plaintext plus a server-checked password (slice 8);
  slice 12 then builds a UI for that capability. Mitigated structurally, but permanent in practice.
...
- **Bloat, not size, is the operational risk.** 100% insert-and-delete churn outruns default
  autovacuum; the sweeper fails silently, so slice 9 surfaces it.
...
- **The anonymous tier has nobody to hold accountable, permanently.** IPs are free, suspension is
  partial by construction, and slice 12's panel is account-shaped.
...
**Anonymous abuse response.** What does it look like with no account to suspend? → slices 12 and 13
(the one question that could argue back into slice 5)
...
to its live clips? → slice 12.

**Still unowned.** Is `curl host/notes` a first-class interface — does the read URL serve raw text to
a non-browser client? (Half-answered: a bare `GET` never destroys anything.) → slices 8, 14. Is there
a legal or takedown posture beyond the abuse queue? → slices 12, 14.
