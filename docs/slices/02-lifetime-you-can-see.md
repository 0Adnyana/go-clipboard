...
**Goal:** lifetime stops being a constant and becomes something the user picks, watches, and can
extend. The create form grows up.

**User story:** *I'm moving a password, not notes, so I pick 10 minutes instead of a day — and I can
see it counting down. When I'm not finished with it, I extend it.*
...
- **Extending is an explicit action, bounded by the clip's own ceiling — 2 hours from creation
  here.** Extending a clip made 110 minutes ago buys ten minutes, not a fresh preset. Extending an
  already-expired clip fails rather than resurrecting it.
...
- Ownership, so extend is available to anyone holding the URL. → slice 5
...
- **A grace period on reclaim.** Expired slugs are instantly claimable, so someone who hits extend a
  second too late finds their name gone. A short window where the slug is dead but not yet claimable
  would fix that. The extend button is what makes this failure reachable, which is why the question
  lands here.
